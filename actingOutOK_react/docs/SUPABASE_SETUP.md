...
**If env vars are set but the site still shows old/deleted data:** After redeploying, open your production site → **DevTools → Network** → reload the Casting Calls page → click the request to `/api/data/casting-calls` → check **Response Headers** for **`X-Data-Source`** and **`X-List-Count`**. If **`X-Data-Source`** is **`json`**, the server is not using Supabase (env not applied, or wrong env scope). If it is **`supabase`**, compare **`X-List-Count`** with the number of rows in Supabase → Table Editor → **casting_calls**. If the counts match, the API is returning exactly what’s in the table, so the “extra” casting call (e.g. Mad Jackal) is still in that project—open the project that **SUPABASE_URL** points to, find the row (e.g. slug `mad-jackal-2026`), and delete it there. If **X-List-Count** is *higher* than your table row count, something else is wrong (e.g. cache); try **Redeploy** with **Clear cache** in Vercel.
