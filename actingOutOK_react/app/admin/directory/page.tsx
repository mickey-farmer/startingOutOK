...
export default function AdminDirectoryPage() {
    ...
    try {
      const res = await fetch("/api/admin/save", {
        ...
        body: JSON.stringify({
          ...
          message: "Admin: update directory",
        }),
      });
  ...
  if (loading || !data) {
    return (
      <div>
        <h1 className="admin-page-title">Directory</h1>
    ...
    <>
      ...
      <div className="admin-card" style={{ marginBottom: "1rem" }}>
        ...
        <p style={{ margin: "0.5rem 0 0", fontSize: "0.85rem", color: "var(--color-muted)" }}>
          Saves to GitHub; the site will update after the next deploy.
        </p>
      </div>
      {addingSection ? (
        ...
        <div key={section} className="admin-card">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.75rem" }}>
            ...
            <button
              ...
              onClick={() => {
                if (confirm(`Remove section "${section}" and all its entries?`)) removeSection(section);
              }}
            >
              Remove section
            </button>
              ...
              <li key={`${entry.id}-${index}`} style={{ marginBottom: "0.5rem", padding: "0.5rem", background: "rgba(0,0,0,0.03)", borderRadius: 6 }}>
                {editing?.section === section && editing?.index === index ? (
                  <EntryForm
                    entry={entry}
                    onSave={(updates) => {
                      updateEntry(section, index, updates);
                      setEditing(null);
                    }}
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    ...
                    <div style={{ display: "flex", gap: "0.25rem" }}>
                      ...
                      <button
                        ...
                        onClick={() => { if (confirm("Remove this entry?")) removeEntry(section, index); }}
                      >
                        Remove
                      </button>
                    </div>
          ...
          <AddEntryForm
            section={section}
            onAdd={(entry) => addEntry(section, entry)}
          />
        </div>
      ))}
    </>
  ...
  return (
    <div className="admin-card" style={{ marginTop: "0.5rem" }}>
      ...
      <div className="admin-form-group">
        ...
        <input
          value={form.pronouns ?? ""}
          onChange={(e) => setForm((f) => ({ ...f, pronouns: e.target.value || null }))}
          placeholder="he/him"
        />
      </div>
      <div className="admin-form-group">
      ...
      <div className="admin-form-group">
        <label>Link (e.g. IMDb) (optional)</label>
        <input
          value={form.link ?? ""}
      ...
      <div className="admin-form-group">
        ...
        <input
          ...
          onChange={(e) => setForm((f) => ({ ...f, contactLink: e.target.value || null }))}
          type="url"
        />
      </div>
      <div className="admin-form-group">
        ...
        <input
          ...
          onChange={(e) => setForm((f) => ({ ...f, contactLabel: e.target.value || null }))}
          placeholder="Instagram"
        />
      </div>
      <div className="admin-form-group">
        <label>Pills (comma-separated, optional)</label>
        <input
          value={(form.pills || []).join(", ")}
          onChange={(e) =>
            setForm((f) => ({
              ...f,
              pills: e.target.value ? e.target.value.split(",").map((s) => s.trim()).filter(Boolean) : undefined,
            }))
          }
          placeholder="Beginner, Available"
        />
      </div>
      <div style={{ display: "flex", gap: "0.5rem" }}>
